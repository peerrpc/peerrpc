...
func runSignal(_ *cobra.Command, _ []string) error {
	...
	case signalFlags.jwtSecret != "":
		validator = jwtVerifierAdapter{secret: []byte(signalFlags.jwtSecret)}
...
// jwtVerifierAdapter bridges the signaling server's TokenValidator
// (which yields an Identity) to the JWT crypto layer in the auth
// package (HS256Verifier, which yields Claims). It extracts the
// standard claim fields and converts.
...
func (a jwtVerifierAdapter) Validate(_ context.Context, token string) (auth.Identity, error) {
	v := auth.HS256Verifier{Secret: a.secret}
