/**
 * v2 signal client for the connect-go signaling server.
 *
 * Replaces v1's ConnectSignal with a service-based target. Wire
 * format matches proto/peerrpc/signaling/v2/signaling.proto.
 *
 * The original v1 ConnectSignal is kept as ConnectSignalV1 for
 * callers that have not migrated; it will be removed two releases
 * after v2 GA per Q1.
 */
...
export interface ConnectSignalV2Config {
  /** Signal-server base URL (e.g. "https://signal.example.com"). */
  url: string;
  /** Rendezvous key (replaces v1 roomId). */
  service: string;
